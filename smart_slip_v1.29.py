...
def calculate_clean_et(et, conditions):
    """
    Estimate what the ET would have been on a clean pass.
    Positive adjustment = Clean ET is slower than actual.
    Negative adjustment = Clean ET is faster than actual.
    """
    adjustment = 0.0
    
    # Nitrous = benefit (Clean ET should be slower than actual if nitrous was used)
    if conditions.get("sprayed_before_330") or conditions.get("sprayed_after_330"):
        adjustment += 0.08   # Without nitrous, car would be slower
    
    severity_multipliers = {"mild": 0.6, "moderate": 1.0, "severe": 1.5}
    
    # Problems = penalty (Clean ET should be faster/better than actual)
    ...
    # Brakes after 330' = penalty (car was slowed down)
    # Clean ET (without brakes) should be faster than actual
...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.29 • All Time Slip Data + Corrected Brakes Logic")
...
CRITICAL RULES:
- Pull historical weather for the exact time on the slip (check airdensityonline.com if needed).
- Output ONLY key=value lines. One key per line. NO extra text at all.
...
# PREDICT + GROK (uses all saved time slip data + all runs)
with tab_predict:
    st.subheader("Predict ET (Uses All Saved Data)")
    ...
    col_icao, col_fetch = st.columns([3, 1])
    with col_icao:
        pred_icao = st.text_input("Airport ICAO", value="KMDT", key="pred_icao")
    with col_fetch:
    ...
    with col_t1:
        target_temp = st.number_input("Temp °F", value=default_temp, step=0.5, key="target_temp")
    with col_t2:
        target_altim = st.number_input("Altimeter", value=default_altim, step=0.01, format="%.2f", key="target_altim")
    with col_t3:
        target_hum = st.number_input("Humidity %", value=default_hum, step=1, key="target_hum")

    target_da = calculate_da(target_temp, target_altim, target_hum)
    st.caption(f"**Target Density Altitude: {target_da} ft**")

    if st.button("Calculate Prediction (All Data)", type="primary", use_container_width=True):
        ...
        if not model or len(vehicle_runs) < 3:
            ...
            if baseline.get("density_altitude"):
                ...
                st.caption(f"Using ALL {model['runs_used']} runs with full time slip data (R² = {model['r2']})")
...
# HISTORY (shows all time slip increments)
with tab_history:
    st.subheader("History (All Time Slip Data Saved)")
    ...
    if not runs:
        ...
        if not st.session_state.show_all_history:
            ...
            st.caption(f"Showing last {len(display_runs)} runs with all increments")
            if st.button("Show All Runs"):
                st.session_state.show_all_history = True
                st.rerun()
        else:
            display_runs = runs[::-1]
            st.caption(f"Showing all {len(display_runs)} runs with all increments")
...
with tab_settings:
    ...
    st.caption("v1.29 • All Time Slip Increments Saved + Corrected Brakes Logic")

st.divider()
st.caption("Smart Slip v1.29 • Built for bracket racers")