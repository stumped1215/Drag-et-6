...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("Bracket Racing • Formula + Grok Hybrid  •  v1.12")
...
with tab_import:
    ...
    car_number = st.text_input("Car Number (optional but recommended)", 
                               placeholder="e.g. 1258 or Right Lane",
                               help="Enter the car number so Grok knows which side of the slip to read.")

    st.markdown("**How to import a timeslip:**")
    st.markdown("1. Take a clear photo of your timeslip.")
    st.markdown("2. Upload the photo to Grok (me) in this chat.")
    st.markdown("3. Copy and paste the prompt below (it will include your car number if you entered it).")

    base_prompt = """Please extract all data from this timeslip photo and format it as a clean import block for Smart Slip.
...
Format everything as key=value lines, one per line."""
    ...
    if car_number.strip():
        full_prompt = f"The car number is **{car_number.strip()}**. Please extract the data from the correct side of the slip for this car.\n\n" + base_prompt
    else:
            ...
            if not block or not block.strip():
                ...
                if "et" not in data:
                    ...
                    new_run = {
                        ...
                        "vehicle": data.get("vehicle", car_number.strip() or "Main Car"),
                        "et": data["et"],
...
# MANUAL LOG (reordered fields + full special conditions)
with tab_manual:
    ...
    with col2:
        ...
        with colA:
            spin = st.checkbox("Spin", key="manual_spin")
            lift_before = st.checkbox("Lift before 330'", key="manual_lift_before")
            lift_after = st.checkbox("Lift after 330'", key="manual_lift_after")
        with colB:
            brakes_after = st.checkbox("Brakes after 330'", key="manual_brakes")
            sprayed_before = st.checkbox("Sprayed (nitrous) before 330'", key="manual_sprayed_before")
            sprayed_after = st.checkbox("Sprayed (nitrous) after 330'", key="manual_sprayed_after")

        notes = st.text_area("Notes (optional)")
        adjustment = 0.0
        if sprayed_before or sprayed_after: adjustment -= 0.08
        if lift_before or lift_after: adjustment += 0.05
...
# PREDICT + GROK (with live weather option)
with tab_predict:
    ...
    # Live weather section for prediction
    st.markdown("**Target Conditions**")
    col_w1, col_w2 = st.columns([2, 1])
    with col_w1:
        pred_icao = st.text_input("Airport ICAO for live weather", value="KMDT", key="pred_icao")
    with col_w2:
        if st.button("Fetch Current Weather", key="pred_fetch"):
    ...
    # Use session state for live values if available
    default_temp = st.session_state.get("pred_temp", 78.0)
    default_altim = st.session_state.get("pred_altim", 29.92)

    col_t1, col_t2 = st.columns(2)
    with col_t1:
        target_temp = st.number_input("Target Temp °F", value=default_temp, step=0.5, key="target_temp")
    with col_t2:
        target_altim = st.number_input("Target Altimeter", value=default_altim, step=0.01, format="%.2f", key="target_altim")

    target_hum = st.number_input("Target Humidity %", value=50, step=1, key="target_hum")
    target_da = calculate_da(target_temp, target_altim, target_hum)
    st.caption(f"Calculated Target DA: **{target_da} ft**")
...
st.divider()
st.caption("Smart Slip v1.12 • Formula + Grok Hybrid Intelligence • Built for bracket racers")