...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("Bracket Racing • Formula + Grok Hybrid  •  v1.5")
...
with tab_import:
    ...
    **How to import a timeslip:**
    ...
    3. Copy and paste this prompt when sending the photo:
    """)
...
Also pull the weather for the time shown on the slip from airdensityonline or aviation sources and include:
- temp_f
- altimeter_inhg
- humidity_pct
- density_altitude
...
Format everything as key=value lines, one per line."""
    ...
    st.caption("Copy the prompt above and send it with your timeslip photo.")

    if not st.session_state.import_success:
        block = st.text_area("Paste the formatted block from Grok here:", height=200, key="import_block")
    ...
    if st.button("Save Run", type="primary"):
        if et <= 0:
            ...
            new_run = {
                ...
                "adjusted_et": et + adjustment if adjustment != 0 else None
            }
            runs.append(new_run)
            save_runs()
...
with tab_predict:
    ...
    if st.button("Generate Full Prompt for Grok", type="primary"):
        if not vehicle_runs:
            ...
            prompt += "\nPlease give a smart ET prediction with reasoning and a recommended safe dial-in."
            
            st.code(prompt, language="text")
            st.caption("Copy the prompt above and send it to me. Paste my response in the box below to save it.")
...
st.divider()
st.caption("Smart Slip v1.5 • Formula + Grok Hybrid Intelligence • Built for bracket racers")