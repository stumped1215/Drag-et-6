...
# ====================== UI ======================
st.title("🏁 Smart Slip")
st.caption("v2.3.2 • PWA + Photo Import + Grok API")
...
tab_photo, tab_manual, tab_predict, tab_history, tab_settings = st.tabs([
    "Photo Import", "Manual Log", "Predict + Grok", "History", "Settings"
])
...
with tab_manual:
    ...
    if st.button("Pull Current Weather", key="manual_pull_wx"):
        ...
        if wx:
            ...
            st.success(f"Loaded weather from {icao}: {wx.get('temp_f')}°F / {wx.get('altimeter_inhg')} inHg")
            st.rerun()
        else:
    ...
    with c2:
        altim = st.number_input("Altimeter", value=st.session_state.get("manual_altim_val", 29.92), step=0.01, format="%.2f", key="manual_altim")
    with c3:
    ...
    if not all_vehicles:
        ...
        if st.button("Pull Current Weather", key="pred_wx"):
            ...
            if wx:
                ...
                st.success(f"Loaded: {wx.get('temp_f')}°F / {wx.get('altimeter_inhg')} inHg")
                st.rerun()
            else:
                st.error(f"Could not get weather for {icao}. Check the ICAO code.")
...
st.divider()
st.caption("Smart Slip v2.3.2 • PWA + Photo Import + Grok API")
