...
def send_reset_email(to_email: str, code: str):
    try:
        ...
        frm = st.secrets.get("SMTP_FROM", user)
    except Exception:
        host = user = pw = frm = ""
        port = 587
    if not host or not user or not pw:
        return False, "Email sending is not set up yet. Add SMTP_HOST, SMTP_USER, SMTP_PASSWORD to Secrets."
    msg = MIMEText(
...
# ====================== UI ======================
st.title("🏁 Smart Slip")
st.caption("v2.6 • Email login + password reset")

# ---------- Cookies / stay logged in ----------
cookies = None
if COOKIE_AVAILABLE:
    cookies = CookieManager(key="ss_cookies")
...
st.divider()
st.caption("Smart Slip v2.6 • Email login + password reset")
