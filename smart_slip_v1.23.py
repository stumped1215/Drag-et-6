...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.23 • Polish Update")
...
# Dark mode toggle in settings will control this
...
with tab_import:
    ...
    prompt = f"""The car number is {car_number or '____'}. Please extract the data from the correct side of the slip and format it for Smart Slip.

Fields: date, time, track, vehicle, et, sixty_ft, eighth_et, trap_mph, reaction_time, temp_f, altimeter_inhg, humidity_pct, density_altitude, notes"""
...
# MANUAL LOG (with last run defaults)
with tab_manual:
...
with tab_history:
    ...
    if not runs:
        ...
        for idx, r in enumerate(display_runs):
            with st.expander(f"{r['date']} • {r.get('vehicle','')} • {r['et']:.3f}s"):
                ...
                if st.button("Delete this run", key=f"delete_{idx}"):
...
with tab_settings:
    ...
    if st.session_state.dark_mode_toggle != st.session_state.dark_mode:
...
st.divider()
st.caption("Smart Slip v1.23 • Built for bracket racers")