...
# ====================== UI ======================
st.title("🏁 Smart Slip")
st.caption("v2.1.1 • Multi-user • Google Sheets • Per-car predictions")
...
with tab_settings:
    ...
    with st.expander("New Profile"):
        ...
        weight = st.number_input("Weight (lbs)", value=2200, step=50, key="profile_weight")
        tire_size = st.text_input("Tire Size", value="28x10.5", key="profile_tire_size")
        tire_type = st.selectbox("Tire Type", ["Radial", "Bias"], key="profile_tire_type")
        first_gear = st.number_input("1st Gear Ratio", value=2.50, step=0.05, format="%.2f", key="profile_first_gear")
        rear_gear = st.number_input("Rear Gear Ratio", value=4.10, step=0.05, format="%.2f", key="profile_rear_gear")
...
""")
    ...
    if not GSPREAD_AVAILABLE:
        st.error("`gspread` or `google-auth` is not installed. Add them to requirements.txt")
    else:
        st.write("Libraries loaded: OK")
        if "gcp_service_account" not in st.secrets:
            st.error("No `[gcp_service_account]` found in Streamlit Secrets")
        else:
            st.write("Secrets found: OK")
            try:
                client = get_gspread_client()
                if client is None:
                    st.error("Client creation returned None (check secrets format)")
                else:
                    st.write("Client created: OK")
                    try:
                        sheet = client.open(SHEET_NAME)
                        st.success(f"Google Sheets connected! Found spreadsheet: **{SHEET_NAME}**")
                    except Exception as e:
                        st.error(f"Could not open spreadsheet '{SHEET_NAME}': {e}")
                        st.info("Make sure the Sheet is named exactly SmartSlipData and is shared with the service account email as Editor.")
            except Exception as e:
                st.error(f"Auth / connection error: {e}")

st.divider()
st.caption("Smart Slip v2.1.1 • Each user’s data stays completely separate")