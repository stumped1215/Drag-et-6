...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.36 • Car Profiles + Notes Focused")
...
# IMPORT FROM GROK (with profile selection)
with tab_import:
...
# MANUAL LOG (with profile selection)
with tab_manual:
    ...
    if car_profiles:
        ...
        default_vehicle = selected_profile["name"] if selected_profile else "Main Bracket Car"
    else:
        manual_profile_id = None
        selected_profile = None
        default_vehicle = "Main Bracket Car"
        st.warning("No car profiles yet. Create one in Settings.")

    vehicle = st.text_input("Vehicle Name", value=last_run.get("vehicle", default_vehicle))
...
with tab_predict:
    ...
    if all_vehicles:
        selected_vehicle = st.selectbox("Select Vehicle for Prediction", all_vehicles, index=0)
    ...
    if st.button("Calculate Prediction", type="primary", use_container_width=True):
        ...
        if not model or len(vehicle_runs) < 3:
            ...
            da_diff = target_da - baseline.get("density_altitude", 0)
            predicted = baseline_et + (da_diff / 1000) * model["sensitivity"]
            st.success(f"**Predicted ET for {selected_vehicle}: {predicted:.3f}s**")
            st.caption(f"Using {model['runs_used']} runs for this vehicle (R² = {model['r2']})")
    ...
    if st.button("Generate Prompt for Grok", use_container_width=True):
        if vehicle_runs:
            ...
            # Add profile info if available from most recent run
...
# SETTINGS (removed Default Vehicle and Safety Margin)
with tab_settings:
...
st.divider()
st.caption("Smart Slip v1.36 • Built for bracket racers")