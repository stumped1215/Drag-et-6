...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("v1.28 • Full History Editing + All Data for Predictions")
...
# PREDICT + GROK (uses ALL data + live weather)
with tab_predict:
    ...
    # Live weather pulling (prominent)
    ...
    if st.button("Calculate Prediction (using all data)", type="primary", use_container_width=True):
        model = calculate_model(vehicle_runs)  # Uses ALL runs with weather data
        if not model or len(vehicle_runs) < 3:
            st.error("Need at least 3 runs with weather data for this vehicle.")
        else:
            ...
            if baseline.get("density_altitude"):
                ...
                st.caption(f"Model uses ALL {model['runs_used']} runs with weather data (R² = {model['r2']})")
...
# HISTORY (fully editable)
with tab_history:
    st.subheader("History (Fully Editable)")
    ...
    if not runs:
        ...
        if not st.session_state.show_all_history:
            ...
            st.caption(f"Showing last {len(display_runs)} runs (fully editable)")
            if st.button("Show All Runs"):
                st.session_state.show_all_history = True
                st.rerun()
        else:
            display_runs = runs[::-1]
            st.caption(f"Showing all {len(display_runs)} runs (fully editable)")
        ...
        if display_runs:
            ...
            # Show editable table with key columns
            editable_cols = ["notes", "actual_et", "clean_et_estimate"]
            display_cols = ["date", "vehicle", "et", "actual_et", "clean_et_estimate", "density_altitude", "track", "notes"]
...
with tab_settings:
    ...
    st.caption("v1.28 • Full History Editing + All Data Used for Predictions")

st.divider()
st.caption("Smart Slip v1.28 • Built for bracket racers")