...
# ============== UI ==============
st.title("🏁 Smart Slip")
st.caption("Bracket Racing • Formula + Grok Hybrid  •  v1.6")
...
# IMPORT FROM GROK (with special condition checkboxes)
with tab_import:
...
Format everything as key=value lines, one per line."""
    ...
    if not st.session_state.import_success:
        ...
        st.markdown("**Special Conditions (if this was not a clean pass):**")
        col1, col2 = st.columns(2)
        with col1:
            spin = st.checkbox("Spin")
            lift = st.checkbox("Lift (after or before 330')")
        with col2:
            brakes = st.checkbox("Brakes (after 330')")
            sprayed = st.checkbox("Sprayed (after or before 330')")
    ...
    if st.button("Generate Full Prompt for Grok", type="primary"):
        if not vehicle_runs:
            ...
            for r in recent:
                ...
                if r.get("lift"): conditions.append("lift")
                if r.get("brakes_after_330"): conditions.append("brakes after 330")
                if r.get("sprayed"): conditions.append("sprayed")
                if conditions:
            ...
            prompt += "\nPlease give a smart ET prediction with reasoning and a recommended safe dial-in. Consider any special conditions listed above."
...
st.divider()
st.caption("Smart Slip v1.6 • Formula + Grok Hybrid Intelligence • Built for bracket racers")